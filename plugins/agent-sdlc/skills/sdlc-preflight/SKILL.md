...
8. On success, update `## Agent State` with `Status: preflight_passed`, base, branch, and any safe non-local-path worktree note.

## Rules

- Do not create implementation changes in this role.
- Do not put local absolute paths in GitHub issue bodies or comments.
- Before `gh issue edit --body-file` or `gh pr edit --body-file`, run `node plugins/agent-sdlc/scripts/guardrails.ts body --body-file <draft> > <safe-body>`.
