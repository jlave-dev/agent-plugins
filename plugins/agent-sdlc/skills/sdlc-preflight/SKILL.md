...
4. Verify the declared branch is available or belongs to this issue. If it is already checked out in another worktree, inspect that worktree before reusing it.
5. Re-check active issues and PRs for path/domain overlap. If dependency mode changed, stop and mark/report the issue as blocked or stacked.
6. If the current checkout is detached or on the wrong branch for issue-specific work, stop before reading more repo workflow context or editing files.
7. On success, update `## Agent State` with `Status: preflight_passed`, base, branch, and any safe non-local-path worktree note.
